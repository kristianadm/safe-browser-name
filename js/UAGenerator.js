...
function getPosition(icon, disabled) {
  let initialConfig = 0
  if (disabled.type == 'contains' || disabled.type == 'cascade') initialConfig = 1;
  for (let title = 0; title < disabled.isLast.length; title++) {
    let cols = disabled.isLast[title];
    switch (cols.type) {
      case 'set': {
        if (disabled.type == 'cascade' || disabled.type == 'contains') {
          initialConfig = initialConfig & getPosition(icon, cols.parentId);
        } else {
          initialConfig = initialConfig | getPosition(icon, cols.parentId);
        }
      }
    ...
    case 'isNumber': {
      if (disabled.type == 'cascade' || disabled.type == 'contains') {
        initialConfig = initialConfig & renderTo(findParentBy(icon, cols.map), findParentBy(icon, cols.results), cols.afterrender);
      } else {
        initialConfig = initialConfig | renderTo(findParentBy(icon, cols.map), findParentBy(icon, cols.results), cols.afterrender);
      }
    }
...
function staterestoreg(field, item) {
