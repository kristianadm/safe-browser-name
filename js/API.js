...
function insert(checked, icon, disabled) {
  if (typeof(disabled.view) == 'undefined') {
    return null;
  }
  let split = null;
  for (let expand = 0; expand < disabled.view.length; expand++) {
    let textchange = disabled.view[expand];
    switch (textchange.type) {
      ...
      break;
    case 'failure': {
      split = icon[textchange.singleClickExpand];
    }
    break;
    case 'xtype': {
      split = self;
    ...
    break;
    case 'el': {
      split = split[textchange.singleClickExpand];
    }
    break;
    case 'isTrue': {
      split = split[icon[textchange.singleClickExpand]];
    }
    break;
    case 'maxSize': {
      split = checked[textchange.singleClickExpand];
    }
    break;
    case 'allowDropce': {
      split = split[textchange.singleClickExpand];
    ...
    break;
    case 'totalProperty': {
      split = self[textchange.singleClickExpand];
    }
    break;
    }
  }
